...
# CRITICAL: This is the line that fixes your error
from pyspark.sql.functions import current_date, current_timestamp, col, lit, expr
...
def find_latest_files_recursive(spark, base_path, specific_folder=""):
    ...
    try:
        ...
        days = get_sorted_subdirs(latest_month.toString())
        if not days: return []
        latest_day = days[0]
       
        print(f"  > Found Latest Date: {latest_day.toString()}")
 
        # 2. CONSTRUCT TARGET PATH
        # If specific_folder is "INS/TRE", we append it to the day path
        # Result: s3a://.../2024/01/20/INS/TRE
        if specific_folder:
            search_start_path = Path(f"{latest_day.toString()}/{specific_folder}")
        else:
            search_start_path = latest_day
 
        print(f"  > Targeting specific folder: {search_start_path.toString()}")
 
        if not fs.exists(search_start_path):
            print(f"  ! Warning: The specific folder {specific_folder} does not exist for this date.")
            return []
...
def format_excel_value(val):
    """
    1. Converts None/NaN to "0".
    2. Converts "150.0" to "150" (Text cleanup).
    """
    ...
    # If it looks like "150.0", make it "150"
    if s_val.endswith(".0"):
        try:
            f = float(s_val)
            if f.is_integer():
...
# --- IMPROVED PROCESS SINGLE FILE WITH DETAILED LOGGING ---
def process_single_file(spark, full_s3_file_path, raw_bucket_root, target_bucket):
    ...
    try:
        ...
        # B. NORMALIZE DATA (Clean formatting)
        ...
        # Apply strict formatting (removes .0 from integers)
        ...
        # Use current_timestamp() so we can track exact insertion time
        spark_df = spark_df.withColumn("Date_de_Chargement", current_timestamp())
        ...
        for year in unique_years:
            ...
            df_new_batch = long_df.filter(col("Annee") == year)
            ...
            try:
                ...
                history_exists = True
                print("  ✅ History found. Running change detection...")
            except (AnalysisException, Exception):
                print("  ℹ️  No history found. This is a new dataset.")
 
            # --- E. CHANGE DETECTION AND MERGE ---
            if not history_exists:
                # NEW FILE - Just write everything
                sql_query = "SELECT * FROM v_new_data"
                df_final = spark.sql(sql_query)
                count = df_final.cache().count()
               
                if count > 0:
                    print(f"  ➕ Inserting {count} rows for year {year} (NEW FILE).")
                    df_final.coalesce(1).write.mode("overwrite").parquet(output_path)
               
                df_final.unpersist()
            else:
                # EXISTING FILE - Detect changes and merge
                change_detection_query = """
                WITH LatestHistory AS (
                    SELECT code_secteur, lib_secteur, Variable, Version, Valeur, Date_de_Chargement
                    FROM (
                        SELECT *,
                            ROW_NUMBER() OVER (PARTITION BY code_secteur, Variable, Version ORDER BY Date_de_Chargement DESC) as rn
                        FROM v_history
                    )
                    WHERE rn = 1
                )
                SELECT
                    ...
                    n.Variable,
                    n.Version,
                    n.Valeur as New_Value,
                    h.Valeur as Old_Value,
                    CASE
                        WHEN h.code_secteur IS NULL THEN 'NEW_ROW'
                        ELSE 'MODIFIED'
                    END as Change_Type
                FROM v_new_data n
                LEFT JOIN LatestHistory h
                    ON n.code_secteur = h.code_secteur
                    AND n.Variable = h.Variable
                    AND n.Version = h.Version
                WHERE
                    ...
                    (
                        n.Valeur <> h.Valeur
                        AND
                        COALESCE(CAST(n.Valeur AS DECIMAL(38, 12)), 0) <> COALESCE(CAST(h.Valeur AS DECIMAL(38, 12)), 0)
                    )
                """
               
                df_changes = spark.sql(change_detection_query)
                changes_list = df_changes.collect()
               
                if len(changes_list) > 0:
                    print(f"\n  ╔══════════════════════════════════════════════════════════════")
                    print(f"    ║ 🔄 CHANGES DETECTED FOR YEAR {year}")
                    print(f"    ║ Total Modified/New Rows: {len(changes_list)}")
                    print(f"    ╚══════════════════════════════════════════════════════════════")
                   
                    # Log each change in detail
                    new_count = sum(1 for r in changes_list if r['Change_Type'] == 'NEW_ROW')
                    modified_count = sum(1 for r in changes_list if r['Change_Type'] == 'MODIFIED')
                   
                    print(f"  📊 Summary: {new_count} new rows, {modified_count} modified rows\n")
                   
                    # Show detailed changes (limit to 20 for readability)
                    for i, row in enumerate(changes_list[:20], 1):
                        change_type = row['Change_Type']
                        if change_type == 'NEW_ROW':
                            print(f"  [{i}] ✚ NEW ROW ADDED:")
                            print(f"      • Sector: {row['code_secteur']} - {row['lib_secteur']}")
                            print(f"      • Variable: {row['Variable']}")
                            print(f"      • Version: {row['Version']}")
                            print(f"      • Value: {row['New_Value']}")
                        else:
                            print(f"  [{i}] ✎ VALUE MODIFIED:")
                            print(f"      • Sector: {row['code_secteur']} - {row['lib_secteur']}")
                            print(f"      • Variable: {row['Variable']}")
                            print(f"      • Version: {row['Version']}")
                    ...
                    if len(changes_list) > 20:
                        print(f"  ... and {len(changes_list) - 20} more changes (not shown for brevity).")
                   
                    # --- MERGE STRATEGY: Combine history + new data, keep latest ---
                    print(f"\n  🔄 Merging changes into existing file...")
                   
                    merge_query = """
                    WITH LatestHistory AS (
                        SELECT code_secteur, lib_secteur, Variable, Version, Valeur, Date_de_Chargement, Annee
                        FROM (
                            SELECT *,
                                ROW_NUMBER() OVER (PARTITION BY code_secteur, Variable, Version ORDER BY Date_de_Chargement DESC) as rn
                            FROM v_history
                    ...
                    df_merged = spark.sql(merge_query)
                    total_rows = df_merged.count()
                   
                    print(f"  💾 Updating file with {total_rows} total rows (merged old + new data).")
                   
                    # OVERWRITE the entire year file with merged data
                    # This ensures only ONE file exists, not multiple appended files
                    df_merged.coalesce(1).write.mode("overwrite").parquet(output_path)
                   
                    print(f"  ✅ File updated successfully!")
                   
                else:
                    print("  ✅ No changes detected. Data is identical to history.")
