...
for file_path in csv_files:
    ...
    transformed_df = (
        ...
        .withColumn("dim_id", lit("N/A"))
        .withColumn("dim_key", lit("N/A"))
...
#     # -------------------------
#     # years = [row["annee"] for row in transformed_df.select("annee").distinct().collect()]
#     years = transformed_df.select("annee").distinct().toLocalIterator()
#     for y in years:
#         df_year = transformed_df.filter(col("annee") == y)
#         output_path = os.path.join(TARGET_BASE, indicator_folder, str(y))
#         log.info(f"💾 Processing year {y}")
...
#             #     SELECT d.*
#             #     FROM df_year d
#             #     LEFT ANTI JOIN existing_df e
#             #     ON d.annee = e.annee
#             #     AND d.Variable = e.Variable
#             #     AND d.pays = e.pays
#             #     AND d.valeur = e.valeur
#             # """)
#             new_count = new_rows.count()
#             log.info(f"🧐 Rows considered new by Spark for year {y}: {new_count}")
#             new_rows.show(20, truncate=False)
...
#         URI = sc._jvm.java.net.URI
#         fs = FileSystem.get(URI(output_path), sc._jsc.hadoopConfiguration())
...
#         # sécurité : supprimer temp si reste
#         if fs.exists(src):
#             fs.delete(src, True)
