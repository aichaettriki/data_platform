...
#         logger.debug("[TITLE] B: strip too thin, skipping.")
#         return None
#     strip = img_np[strip_y1:strip_y2, 0:w]
#     if strip.size == 0:
#         return None
#     try:
#         ocr_result = ocr_engine.ocr(strip, cls=False)
...
#     - Replace comma decimal separator with dot  (e.g. "1,234" → "1.234")
#     - Strip surrounding whitespace
#     """
#     v = str(value).strip()
#     if v in ('nan', 'None', ''):
#         return ''
#     # Replace comma used as decimal separator ONLY when surrounded by digits
#     # e.g. "1,5" → "1.5"  |  "New York, USA" stays unchanged
#     v = re.sub(r'(?<=\d),(?=\d)', '.', v)
#     return v


# def _clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
#     """Apply _clean_cell to every cell in the dataframe."""
#     return df.applymap(_clean_cell)


# def _fix_duplicate_columns(columns) -> list:
#     """
#     Remove pandas auto-suffix for duplicate column names.
#     e.g. ["2001", "2001.1", "2001.2"] → ["2001", "2001", "2001"]

#     pandas adds ".1", ".2" etc. when two columns share the same name.
#     We detect this pattern and strip it off.
#     """
#     # Pattern: ends with ".<integer>" AND the base name already appeared
#     suffix_re = re.compile(r'^(.+)\.(\d+)$')
#     seen = {}
#     result = []
#     for col in columns:
#         col_str = str(col)
#         m = suffix_re.match(col_str)
#         if m:
#             base = m.group(1)
#             # Only strip if the base name was already seen (confirming it's a dup)
#             if base in seen:
#                 result.append(base)
#                 logger.debug("[ColFix] '%s' → '%s'", col_str, base)
#                 continue
#         seen[col_str] = True
...
#     Fixes applied:
#     - Header row is NOT duplicated into data rows
#     - Duplicate column name suffixes (.1, .2) are stripped
#     - Comma decimal separators are converted to dots (1,5 → 1.5)
#     """
#     engines = _get_engines()
#     layout_engine = engines['layout']
#     table_engine  = engines['table']
...
#                 try:
#                     # ── FIX 1: read WITHOUT promoting first row to header ──────
#                     # header=None means pandas uses 0,1,2... as column names
#                     # and ALL rows (including the real header) stay as data rows
#                     # so we never lose or duplicate the header row.
#                     df_raw = pd.read_html(html_content, header=None)[0]
#                 except Exception as exc:
#                     logger.warning("[Table] pd.read_html failed: %s", exc)
#                     continue

#                 # ── FIX 2: strip duplicate-column suffixes (.1, .2 ...) ───────
#                 # These come from pandas deduplication when two columns share
#                 # the same name (e.g. "2001" appearing twice → "2001", "2001.1")
#                 fixed_cols = _fix_duplicate_columns(df_raw.columns.tolist())
#                 df_raw.columns = fixed_cols

#                 # ── FIX 3: replace comma decimal separator with dot ───────────
#                 # "1,5" → "1.5", "3,14" → "3.14"
#                 # Commas inside text (e.g. "New York, USA") are left untouched
#                 df_clean = _clean_dataframe(df_raw)

#                 if not is_valid_table(df_clean):
#                     logger.info("[Table] p%d: '%s' failed validation — skipped.",
#                                 page_num, display_title)
#                     continue

#                 table_count += 1
#                 str_df = df_clean.astype(str).replace({'nan': '', 'None': ''})
...
#                 })
#                 logger.info("[Table] Saved table_%d: '%s' (%dr x %dc)",
#                             table_count, display_title, *df_clean.shape)
...
def _fix_duplicate_columns(columns) -> list:
    ...
    Remove pandas auto-suffix for duplicate column names.
    e.g. ["2001", "2001.1", "2001.2"] → ["2001", "2001", "2001"]
    Only strips suffix when the base name was already seen.
    """
    suffix_re = re.compile(r'^(.+)\.(\d+)$')
    seen = {}
    result = []
...
def _parse_html_table(html_content: str):
    """
    Parse HTML table into (header_row, df_data) where:
    - header_row: list of column name strings (the actual table header)
    - df_data:    DataFrame of data rows only (no header row inside)

    Strategy:
    1. Use pd.read_html(header=0) → first row becomes column names.
    2. Fix duplicate column suffixes (.1, .2 ...) on column names.
    3. Clean comma → dot in both header and data cells.

    This way the header is preserved as column names (returned separately
    as header_row), and data rows contain only actual data.
    """
    # Step 1: parse with first row as header
    df = pd.read_html(html_content, header=0)[0]

    # Step 2: fix duplicate column name suffixes
    fixed_cols = _fix_duplicate_columns(df.columns.tolist())
    df.columns = fixed_cols

    # Step 3: clean comma→dot on column names too
    header_row = [_clean_cell(str(c)) for c in df.columns.tolist()]

    # Step 4: rename df columns to cleaned names
    df.columns = header_row

    # Step 5: clean all data cells
    df_clean = _clean_dataframe(df)

    return header_row, df_clean
...
def is_valid_table(df: pd.DataFrame) -> bool:
    ...
    # Work on numpy array to avoid ALL duplicate-column ambiguity issues
    # str_arr shape: (rows, cols)
    str_arr = df.astype(str).values
    str_arr[str_arr == 'nan']  = ''
    str_arr[str_arr == 'None'] = ''

    # 1. Fill ratio >= 50%
    fill_ratio = (str_arr != '').sum() / (rows_count * cols)
    if fill_ratio < 0.50:
        logger.debug("[Validate] Rejected: fill ratio %.2f", fill_ratio)
        return False

    # 2. At least 2 diverse columns (nunique >= 2) — iterate by position
    diverse_cols = sum(
        1 for j in range(cols)
        if len(set(v for v in str_arr[:, j] if v)) >= 2
    )
    if diverse_cols < 2:
        logger.debug("[Validate] Rejected: only %d diverse column(s)", diverse_cols)
        return False

    # 3. Row fill consistency (std/mean < 0.6)
    row_fill = (str_arr != '').sum(axis=1).astype(float)
    if row_fill.mean() > 0:
        if row_fill.std() / row_fill.mean() > 0.6:
            logger.debug("[Validate] Rejected: inconsistent row fill")
            return False

    # 4. Header row must not be all-numeric
    first_row = [v for v in str_arr[0].tolist() if v]
    if first_row and all(
        v.replace('.', '', 1).replace('-', '', 1).isdigit() for v in first_row
    ):
        logger.debug("[Validate] Rejected: all-numeric header row")
    ...
    # 5. At least 2 columns present in >50% of rows — iterate by position
    cols_present_majority = sum(
        1 for j in range(cols)
        if (str_arr[:, j] != '').sum() / rows_count > 0.50
    )
    if cols_present_majority < 2:
        logger.debug("[Validate] Rejected: fewer than 2 columns in >50%% of rows")
        return False
    ...
    Output per table:
        {
          "id":     "table_1",
          "page":   3,
          "title":  "Table 2.1 — GDP by sector",
          "header": ["Country", "2001", "2002", "2003"],   ← header row
          "rows":   [["France", "1.5", "1.7", "2.0"], ...]  ← data rows only
        }
    """
    engines = _get_engines()
    layout_engine = engines['layout']
    ...
    for idx, img_np in enumerate(pages):
        ...
        for region in page_table_regions[idx]:
            ...
            for region2 in table_result:
                ...
                try:
                    header_row, df_clean = _parse_html_table(html_content)
                except Exception as exc:
                ...
                tables.append({
                    ...
                    "page":   page_num,
                    "title":  display_title,
                    # header: the actual column names from the table (cleaned)
                    "header": header_row,
                    # rows: data rows only — header is NOT repeated here
                    "rows":   [row.tolist() for _, row in str_df.iterrows()]
                })
                logger.info("[Table] Saved table_%d: '%s' (%dr x %dc) header=%s",
                            table_count, display_title, *df_clean.shape, header_row)
